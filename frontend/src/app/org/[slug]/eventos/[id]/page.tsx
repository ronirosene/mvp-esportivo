...
export default function OrgEventoDetail() {
  ...
  return (
    <div className="space-y-6">
      <button className="text-sm text-muted-foreground hover:text-foreground" onClick={() => router.push(`/org/${slug}/eventos`)}>&larr; Voltar</button>
      <div>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">{new Date(event.dataInicio).toLocaleDateString('pt-BR')} a {new Date(event.dataFim).toLocaleDateString('pt-BR')}</p>
      </div>
      <div>
        ...
        {sports.length === 0 ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {sports.map((es) => (
              <div key={es.id} className="cursor-pointer rounded-lg border p-4 transition-colors hover:bg-muted/50"
                onClick={() => router.push(`/org/${slug}/eventos/${event.id}/modalidades/${es.id}`)}>
