...
export default function OrgHome() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await eventsApi.list({ orgSlug: slug });
        ...
        setCounts(c);
        try {
          const td = await api<any[]>(`/public/schedule/today?orgSlug=${slug}`);
          setTodayMatches(td);
        } catch {}
        try {
          const r = await rankingApi.get({ orgSlug: slug });
  ...
  return (
    <div className="space-y-8">
      <section className="text-center sm:text-left">
        ...
        <p className="mt-2 text-muted-foreground">Acompanhe resultados, classifica&ccedil;&atilde;o e jogos ao vivo.</p>
      </section>
        ...
        {todayMatches.length > 0 && (
          <section>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button className="text-xs font-medium text-muted-foreground hover:text-foreground" onClick={() => router.push(`/org/${slug}/agenda`)}>
                Ver Agenda Completa
              </button>
            </div>
        ...
        {topRanking.length > 0 && (
          <section>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button className="text-xs font-medium text-muted-foreground hover:text-foreground" onClick={() => router.push(`/org/${slug}/ranking`)}>
                Ver Ranking Completo
              </button>
            </div>
            <div className="space-y-1.5">
              {topRanking.map((entry, i) => (
                <div key={entry.cityId} className="flex cursor-pointer items-center justify-between rounded-lg border px-4 py-2.5 text-sm transition-colors hover:bg-muted/30"
                  onClick={() => router.push(`/org/${slug}/cidades/${entry.cityId}`)}>
                  <div className="flex items-center gap-3">
                    <span className="w-5 text-center font-bold">{i === 0 ? '\u{1F947}' : i === 1 ? '\u{1F948}' : i === 2 ? '\u{1F949}' : entry.position}</span>
          ...
          list.length === 0 ? null : (
            <section key={status}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {list.map((evt) => (
                  <div key={evt.id} className="cursor-pointer rounded-lg border p-4 transition-colors hover:bg-muted/50"
                    onClick={() => router.push(`/org/${slug}/eventos/${evt.id}`)}>
                    <h3 className="font-medium">{evt.nome}</h3>
                    <p className="mt-1 text-xs text-muted-foreground">{evt.ano} &middot; {evt.cidadeSede}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{new Date(evt.dataInicio).toLocaleDateString('pt-BR')} a {new Date(evt.dataFim).toLocaleDateString('pt-BR')}</p>
                    {counts[evt.id] && (
