...
export default function EditSponsorPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await sponsorsApi.get(params.id as string);
        setNome(data.nome);
        ...
        setError(err instanceof Error ? err.message : 'Erro ao carregar patrocinador');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      await sponsorsApi.update(params.id as string, {
        nome,
  ...
  return (
    <div className="mx-auto max-w-md space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold tracking-tight">Editar Patrocinador</h1>
